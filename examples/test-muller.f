...
c
cccc        call getarg(1,config)
cccc        call prina('config=*',config,78)
c
        ir = 10
cccc        open(unit=ir,file=config)
...
cccc        call prinf('ipatchinfo=*',ipatchinfo,npatches)
cccc        call prin2('refineinfo=*',refineinfo,4*npatches)
...
c$$$c
c$$$c
c$$$c       ... construct the coefficients of the first fundamental form
c$$$c
c$$$        call dot_prod3d(dxyzduv(1,1),dxyzduv(1,1),g11)
c$$$        call dot_prod3d(dxyzduv(1,1),dxyzduv(1,2),g12)
c$$$        call dot_prod3d(dxyzduv(1,2),dxyzduv(1,1),g21)
c$$$        call dot_prod3d(dxyzduv(1,2),dxyzduv(1,2),g22)
c$$$c
c$$$        g(1,1)=g11
c$$$        g(1,2)=g12
c$$$        g(2,1)=g21
c$$$        g(2,2)=g22
c$$$c
c$$$cc        call prin2('g=*',g,4)
c$$$c
c$$$        detg=g11*g22-g12*g21
c$$$c
c$$$cc        call prin2('sqrt(det|g|)=*',sqrt(detg),1)
c$$$cc        call prin2('and ds=*',ds,1)
c$$$c
        enddo
      enddo
...
c
     ...
     $     xyztang1s(1,i),xyztang2s(1,i),xyzinfo(1,i))
        enddo
c
c$$$ccc        call prin2('whts=*',whts,npts)
c$$$c
c$$$        cd=0
c$$$        do i=1,npts
c$$$        cd=cd+whts(i)
c$$$        enddo
c$$$c
c$$$        call prin2('sum, whts=*',cd,2)
c$$$        call prin2('whts-4 pi=*',cd-4*pi,2)
c
c
...
c
c
c.. simple test for Muller solver
c
...
c
            ...
            lw = 2*(4*npts)**2+(4*npts)+10
            lw = 2*lw
            allocate( w(lw), stat=ier )
ccc           write(*,*) 'ier=',ier
c
            call prinf('entering cqrdecom, 4 x npts=*',4*npts,1)
...
c
c
          if( itype_solve .eq. 4 ) then
c
            call prinf('entering cgmressq, 4 x npts=*',4*npts,1)
c
            t1=second()
C$        t1=omp_get_wtime()
ccceps=1e-5
...
c
            if( allocated(w) ) deallocate(w)
            allocate( w( 2*(ngmrec*2+5)*(4*npts) ) )
c
...
c
            ...
            call prin2('after cgmressq, errs=*',errs,niter)
            call prin2('in cgmressq, time=*',t2-t1,1)
c
          endif
...
c
c$$$          do i=1,3
c$$$            evec1(1)=(evec(1)-evec0(1))
c$$$            evec1(2)=(evec(2)-evec0(2))
c$$$            evec1(3)=(evec(3)-evec0(3))
c$$$            hvec1(1)=(hvec(1)-hvec0(1))
c$$$            hvec1(2)=(hvec(2)-hvec0(2))
c$$$            hvec1(3)=(hvec(3)-hvec0(3))
c$$$          enddo
c$$$
c$$$          call prin2('absolute error, E=*',evec1,6)
c$$$          call prin2('absolute error, H=*',hvec1,6)
c
          stop
