...
c
c
        subroutine ctriaadap(ier,vert1,vert2,vert3,fun,
...
c       quadrature to integrate a user-supplied complex *16 function on
c       a triangle in the plane (also user-supplied)
c
c                       input parameters:
c
...
c        rint is assumed to be complex *16.
...
c       . . . integrate
c
        call ctrianrec(ier,w(istack),vert1,vert2,vert3,fun,
...
c
c
        subroutine ctrianrec(ier,stack,z1,z2,z3,fun,
     ...
     2      rint,maxdepth,maxrec,numint,iw,numfunev,ww)
        ...
        complex *16 w(1),ww(1),rint,vals(1),value1,value2,value3,value4
c
...
c       n.ge.51) to integrate a user-supplied function on a triangle in
c       the plane (also user-supplied)
c
c                       input parameters:
c
...
c
 1000   continue
c
        call ctriaint1(m,z1,z2,z3,fun,
     1      par1,par2,vals(1),w,ww,numnodes)
...
c
        call ctriaint1(m,zout(1,1,1),zout(1,2,1),zout(1,3,1),fun,
     1      par1,par2,value1,w,ww,numnodes)
        numfunev=numfunev+numnodes
c
        call ctriaint1(m,zout(1,1,2),zout(1,2,2),zout(1,3,2),fun,
     1      par1,par2,value2,w,ww,numnodes)
        numfunev=numfunev+numnodes
c
        call ctriaint1(m,zout(1,1,3),zout(1,2,3),zout(1,3,3),fun,
     1      par1,par2,value3,w,ww,numnodes)
        numfunev=numfunev+numnodes
c
        call ctriaint1(m,zout(1,1,4),zout(1,2,4),zout(1,3,4),fun,
     1      par1,par2,value4,w,ww,numnodes)
        numfunev=numfunev+numnodes
c
        dd=cdabs(value1+value2+value3+value4-vals(j))
c
...
c
c
        subroutine ctriaint1(n,vert1,vert2,vert3,fun,
     1      par1,par2,rint,w,work,numnodes)
        implicit real *8 (a-h,o-z)
        dimension vert1(1),vert2(1),vert3(1)
        complex *16 w(1),work(*),rint
        data nold/-10/
c
...
c
        call ctriaint0(numnodes,w(irnodes),w(iweights),
     1     fun,par1,par2,rint)
c
        return
        end
c
...
c
c
        subroutine ctriaint0(numnodes,rnodes,weights,fun,par1,
     1    par2,rint)
        implicit real *8 (a-h,o-z)
...
c
        call fun(rnodes(1,i),rnodes(2,i),par1,par2,d)
        rint=rint+d*weights(i)
 1400 continue
c
        return
        end
