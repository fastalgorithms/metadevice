cc Copyright (C) 2011: Leslie Greengard and Zydrunas Gimbutas, Vladimir Rokhlin
...
c
     ...
     1      weights,ifinit,w)
        ...
        dimension w(1),vert1(2),
     1      vert3(2),vert2(2),rnodes(1),weights(1)
c
...
c
c
        subroutine triagauc2(n,m,vert1,vert2,vert3,rnodes,
     1      weights,ifinit,w)
        implicit real *8 (a-h,o-z)
...
c       Gaussian quadratures are lined up parallel to the side of 
c       the triangle opposite the vertex vert1; the total number 
c       of nodes and weights to be created is n*m. Actually, this 
c       is simply a memory manager for the subroutine triagau20,
c       which does all the work.
c
c                              input parameters:
c
c  n - the number of nodes in vertical direction. 
c  m - the number of nodes in horizontal direction. 
c  Note that the 
c       total number of nodes to be created is n*m, and the 
c       algebraic order of the quadrature is (n-1)*(m-1)
c  vert1,vert2,vert3 - the vertices of the triangle on which the 
c       quadrature rule is to be constructed
c
c                              output parameters:
c
c  rnodes - the array of n*m nodes in the plane (all inside the
c       user-supplied traiangle)
c  weights - the quadrature weights corresponding to the nodes rnodes
c
c                              work arrays:
c
c  w - must be at least 4*(n+m)+5 real *8 locations 
c
c
c        . . . allocate memory in the work array
c
...
c       construct the quadrature formula on the triangle
c
        call triagauc20(n,m,vert1,vert2,vert3,rnodes,weights,
     1      ifinit,w(it),w(iwhts),w(it2),w(iwhts2),
     $      w(itvert),w(iwhtsver) )
c
        return
        end
...
c
c
        subroutine triagauc20(n,m,vert1,vert2,vert3,rnodes,weights,
     1      ifinit,t,whts,t2,whts2,tvert,whtsvert)
        implicit real *8 (a-h,o-z)
        dimension w(1),z1(2),z2(2),z3(2),vert1(1),
     1      vert3(1),vert2(1),rnodes(2,m,n),weights(m,n)
c
        dimension t(1),whts(1),t2(1),whts2(1),tvert(1),whtsvert(1)
c
c       this subroutine constructs a tensor product Gaussian
c       quadrature formula on the triangle in the plane 
c       specified by its vertices. Note that the one-dimensional 
c       Gaussian quadratures are lined up parallel to the side of 
c       the triangle opposite the vertex vert1; the total number 
c       of nodes and weights to be created is n*m.
c
c                              input parameters:
c
c  n - the number of nodes in vertical direction. 
...
c
        ...
        ifwhts=1
        call legewhts(n,t,whts,ifwhts)
        call legewhts(m,t2,whts2,ifwhts)
        endif
c
c       construct affine transformation putting one side
c       of the user-specified triangle on the real axis. The side
...
c
        ...
        rnodes(2,j,i)=y
c
        weights(j,i)=d*whts2(j)*whtsvert(i)
c
 1400 continue
 2000 continue
c
...
c
        ...
        implicit real *8 (a-h,o-z)
        dimension vert1(*),vert2(*),vert3(*),w(*),zin(*),zout(2)
...
c
c       put the original vertices at the top while preserving orientation
c
